...
We present a comparison of four Bayesian approaches to epistemic uncertainty estimation in language models — variational and post-hoc inference across full weights and LoRA adapters — plus a Monte Carlo (MC) Dropout baseline. All methods are evaluated on the same 76M-parameter GPT-2-style architecture trained on The Pile with domain-split OOD evaluation. We report three main findings. First, **LoRA-based Bayesian methods achieved the strongest OOD detection in our setup**: BLoB (Bayesian Low-Rank Adaptation by Backpropagation) LoRA (AUROC 0.909) and TFB (Training-Free Bayesianization) (0.917) outperform full-weight variational inference (0.874) with 17× fewer Bayesian parameters, though this comparison is confounded by differences in training procedure and backbone quality (§6). Second, **diagonal Laplace approximation fails** for language model OOD detection — the diagonal Fisher curvature is flat at convergence regardless of parameterization (full weights or LoRA). Third, **MC Dropout** applied to the deterministic checkpoint (zero extra training) achieves AUROC 0.898 — competitive with trained Bayesian methods (overlapping confidence intervals with BLoB). **TFB**, also requiring no Bayesian training, matches the best trained methods at AUROC 0.917. Production benchmarks show N=3 MC samples capture 97% of the full signal. Code: https://github.com/avdolgikh/bayesian-llm
...
Recent work has introduced several practical methods for Bayesian weight posteriors in large models, including variational LoRA (Bayesian Low-Rank Adaptation by Backpropagation, BLoB; Wang et al., 2024), post-hoc SVD-based variance (Training-Free Bayesianization, TFB; Shi et al., 2025), and post-hoc Laplace on LoRA (Yang et al., 2024). These methods have been evaluated on different architectures, datasets, and metrics, making direct comparison complicated.

We address this gap with a **systematic comparison** on a single architecture and evaluation protocol:
...
All four cells are evaluated on the same 76M-parameter GPT-2-style model trained on The Pile with domain-split evaluation (in-distribution: HackerNews; OOD: ArXiv, FreeLaw, PubMed). We evaluate using AUROC, FPR@95, AUPRC, ECE, Brier score, NLL, and AURC.

![Fig 1. Point Weights vs Bayesian Weight Posteriors](../figures/fig1_point_vs_bayesian.png)

**Contributions:**

1. Head-to-head comparison of variational vs post-hoc Bayesian methods on the same LM architecture, dataset, and evaluation protocol, including an MC Dropout baseline.
...
3. Definitive negative result for **diagonal Laplace** in LM OOD detection — both full-weight and LoRA parameterizations produce MI ratio 1.00× (KFAC and full-Hessian variants were not tested).
4. **MC Dropout** (zero extra training) achieves AUROC 0.898 — a strong baseline whose confidence interval overlaps with trained BLoB LoRA (0.909). **TFB** (also zero Bayesian training) achieves the best AUROC (0.917) via SVD-structured variance on existing LoRA checkpoints.
5. Production deployment recipe: posterior mean weights for serving (zero overhead), N=3 MC for uncertainty scoring (97% of full signal).

![Fig 2. Four Bayesian Methods Compared at 76M Parameters](../figures/fig2_method_matrix.png)
...
These methods report results on different model families (LLaMA, Qwen, GPT-2), different tasks (MMLU, commonsense QA, calibration), and different metrics. Our contribution is an apples-to-apples comparison on a single setup.
...
![Fig 3. BLoB LoRA: $\Delta W = \frac{\alpha}{r} BA$. Only $A$ carries variational posteriors; $B$ is deterministic. KL factorizes in rank-$r$ $A$-space (1.97M Bayesian params, 2.5% of 76M model).](../figures/fig3_blob_lora.png)
...
**Table 1.** OOD detection performance (500 ID + 500 OOD sequences, N=20 MC samples).
...
The two LoRA methods — BLoB (AUROC 0.909) and TFB (0.917) — achieve the strongest OOD detection with overlapping 95% bootstrap CIs. MC Dropout is competitive at 0.898; its CI [0.877, 0.917] overlaps with BLoB [0.890, 0.925], so the advantage of trained variational LoRA is not statistically significant at this sample size. MC Dropout also achieves the best calibration (ECE=0.012) and tied-lowest NLL (2.79). Full-weight variational inference reaches 0.874 but requires 17× more Bayesian parameters. Both diagonal Laplace variants produce near-random detection (AUROC ≈ 0.5) with degraded calibration (Brier ≈ 1.0, NLL > 9.0).
...
**N=3 as the operating point.** AUROC increases from 0.50 (N=1, no disagreement possible) to ~0.86 (N=3) — 97% of the N=20 signal. Further samples yield diminishing returns: N=5→20 contributes <3 AUROC points. We recommend **N=3** as the default operating point (382 MB VRAM).
...
In our comparison of Bayesian uncertainty methods for language models, LoRA-based approaches (BLoB and TFB) achieved the strongest OOD detection at 76M-parameter scale (AUROC 0.909–0.917 vs 0.874 for full-weight variational), though this advantage may partly reflect differences in training procedure and backbone quality rather than parameterization alone. MC Dropout — a zero-cost baseline — proved competitive (AUROC 0.898, overlapping CIs with BLoB). TFB is particularly notable: it achieves the best AUROC (0.917) with zero Bayesian training — only a fast binary search on an existing LoRA checkpoint. Diagonal Laplace approximation fails consistently across all configurations.

For production deployment, we recommend: (1) posterior mean weights for serving predictions (zero overhead — for LoRA, the posterior mean folds into base weights, producing a standard dense model), (2) N=3 MC sampling for uncertainty scoring when needed (97% of full signal, modest VRAM overhead). For applications where marginal AUROC gains over MC Dropout are not critical, MC Dropout on any existing checkpoint offers the lowest-cost path to epistemic uncertainty.
