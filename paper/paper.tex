...
Third, \textbf{MC Dropout} applied to the deterministic checkpoint (zero extra training) achieves AUROC 0.898---competitive with trained Bayesian methods (overlapping confidence intervals with BLoB).
\textbf{TFB}, also requiring no Bayesian training, matches the best trained methods at AUROC 0.917.
Production benchmarks show $N{=}3$ MC samples capture 97\% of the full signal.\footnote{Code: \url{https://github.com/avdolgikh/bayesian-llm}}
\end{abstract}
...
Recent work has introduced several practical methods for Bayesian weight posteriors in large models, including variational LoRA (Bayesian Low-Rank Adaptation by Backpropagation, BLoB; \citealp{wang2024blob}), post-hoc SVD-based variance (Training-Free Bayesianization, TFB; \citealp{shi2025training}), and post-hoc Laplace on LoRA \citep{yang2024bayesian}. These methods have been evaluated on different architectures, datasets, and metrics, making direct comparison complicated.

We address this gap with a \textbf{systematic comparison} on a single architecture and evaluation protocol:
...
\noindent All four cells are evaluated on the same 76M-parameter GPT-2-style model trained on The Pile with domain-split evaluation (in-distribution: HackerNews; OOD: ArXiv, FreeLaw, PubMed). We evaluate using AUROC, FPR@95, AUPRC, ECE, Brier score, NLL, and AURC.
...
\begin{enumerate}
\item Head-to-head comparison of variational vs.\ post-hoc Bayesian methods on the same LM architecture, dataset, and evaluation protocol, including an MC Dropout baseline.
...
\item Definitive negative result for \textbf{diagonal Laplace} in LM OOD detection---both full-weight and LoRA parameterizations produce MI ratio $1.00{\times}$ (KFAC and full-Hessian variants were not tested).
\item \textbf{MC Dropout} (zero extra training) achieves AUROC 0.898---a strong baseline whose confidence interval overlaps with trained BLoB LoRA (0.909). \textbf{TFB} (also zero Bayesian training) achieves the best AUROC (0.917) via SVD-structured variance on existing LoRA checkpoints.
\item Production deployment recipe: posterior mean weights for serving (zero overhead), $N{=}3$ MC for uncertainty scoring (97\% of full signal).
\end{enumerate}

\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{fig2_method_matrix.png}
\caption{Four Bayesian methods compared at 76M parameters. Rows distinguish variational (train-time) from post-hoc (no Bayesian training) approaches; columns distinguish full-weight from LoRA parameterizations. AUROC values shown for OOD detection.}
...
\caption{BLoB LoRA architecture: $\Delta W = \frac{\alpha}{r} BA$. Only $A$ carries variational posteriors ($\mu_A$, $\sigma_A$); $B$ remains deterministic, so the KL divergence factorizes in the rank-$r$ $A$-space (1.97M Bayesian parameters, 2.5\% of the 76M model).}
\label{fig:blob-lora}
\end{figure}
...
\begin{table}[t]
\caption{OOD detection performance (500 ID + 500 OOD sequences, $N{=}20$ MC samples). Bold indicates best result per column among Bayesian methods. 95\% bootstrap CIs from 10{,}000 resamples.}
\label{tab:ood}
\centering
\footnotesize
\ra{1.15}
...
\paragraph{$N{=}3$ as the operating point.}
AUROC increases from 0.50 ($N{=}1$, no disagreement possible) to ${\sim}$0.86 ($N{=}3$)---97\% of the $N{=}20$ signal. Further samples yield diminishing returns: $N{=}5 \to 20$ contributes $<$3 AUROC points. We recommend \textbf{$N{=}3$} as the default operating point (382\,MB VRAM).
...
\paragraph{TFB vs.\ diagonal Laplace.}
...
MC Dropout \citep{gal2016dropout} achieves AUROC 0.898 with zero extra training cost, and its confidence interval overlaps with BLoB LoRA. For applications where marginal AUROC gains are not critical, MC Dropout may be the most cost-effective path to epistemic uncertainty.
